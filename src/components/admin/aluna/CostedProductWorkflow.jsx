...
export default function CostedProductWorkflow({ brandName, initialDraft = {}, isExecuting, onApprove, onApproveQuick, onCancel }) {
  ...
  const [product, setProduct] = useState({ name: initialDraft.product_name || '', description: initialDraft.description || '', price: initialDraft.price || '', tags: Array.isArray(initialDraft.tags) ? initialDraft.tags.join(', ') : '' });
  ...
  const proposal = {
    ...
    product: { name: product.name.trim(), description: product.description.trim(), price: Number(product.price), tags: product.tags.split(',').map((tag) => tag.trim()).filter(Boolean), requires_kitchen: true },
  ...
  return (
    <div className="space-y-4">
      <div><h3 className="text-lg font-bold text-gray-950">Producto con costo real</h3><p className="mt-1 text-sm text-gray-600">Primero ingredientes y receta; el producto se crea al final con su costo por porción. La cantidad de compra debe expresarse en la unidad de uso equivalente.</p></div>
      <div className="space-y-3 rounded-2xl border border-gray-200 bg-white p-4"><input value={categoryName} onChange={(event) => setCategoryName(event.target.value)} placeholder="Categoría *" className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm" /><input value={product.name} onChange={(event) => setProduct((current) => ({ ...current, name: event.target.value }))} placeholder="Nombre del producto *" className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm" /><textarea value={product.description} onChange={(event) => setProduct((current) => ({ ...current, description: event.target.value }))} placeholder="Descripción real *" rows={2} className="w-full resize-none rounded-xl border border-gray-200 px-3 py-2.5 text-sm" /><div className="grid grid-cols-2 gap-3"><input type="number" min="1" value={product.price} onChange={(event) => setProduct((current) => ({ ...current, price: event.target.value }))} placeholder="Precio de venta *" className="rounded-xl border border-gray-200 px-3 py-2.5 text-sm" /><input type="number" min="1" value={servings} onChange={(event) => setServings(Number(event.target.value))} placeholder="Porciones" className="rounded-xl border border-gray-200 px-3 py-2.5 text-sm" /></div><input value={product.tags} onChange={(event) => setProduct((current) => ({ ...current, tags: event.target.value }))} placeholder="Etiquetas separadas por coma" className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm" /></div>
      ...
      <button type="button" disabled={!canCreateQuick} onClick={() => setStep('quick_review')} className="w-full rounded-xl border border-blue-200 bg-blue-50 py-3 text-sm font-bold text-blue-800 disabled:opacity-40">No tengo los costos: crear ahora</button>
