...
const clean = (s = "") =>
  String(s)
    .replaceAll("CafǸ", "Café")
    .replaceAll("TǸ", "Té")
    .replaceAll("Infusi��n", "Infusión")
    .replaceAll("Aromǭtica", "Aromática")
    .replaceAll("Hindǧ", "Hindú")
    .replaceAll("cidr��n", "cidrón")
    .replaceAll("probi��ticos", "probióticos");
...
export default function CoffeeSection({ query, onCount, onQuickView }) {
  ...
  const baseInfClean = baseInf.map((it) => ({ ...it, name: clean(it.name), desc: clean(it.desc) }));
